...
export default function NoteVocale({ interventionId }) {
  ...
  const handleFileChange = (e) => { setFichier(e.target.files[0]); setError(''); };
  ...
  const handleTranscrire = async () => {
    ...
    try {
      const response = await axios.post(`http://localhost:3000/api/notes-vocales/${interventionId}`, formData);
      setTranscription(response.data.texte);
    } catch (err) { setError("Erreur de transcription."); } 
    finally { setIsUploading(false); }
  };

  const handleValider = async () => {
    try {
      await axios.put(`http://localhost:3000/api/notes-vocales/valider/${interventionId}`, { nouveauTexte: transcription });
      setIsValidated(true);
      alert("✅ Note vocale validée !");
    ...
    <div className="mt-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <button onClick={handleTranscrire} disabled={isUploading || !fichier || isValidated} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 disabled:bg-gray-300 text-sm font-medium transition">
          {isUploading ? 'Analyse en cours...' : 'Transcrire'}
        </button>
      </div>
