...
export default function SignatureClient({ interventionId }) {
  ...
  const saveSig = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/signatures/${interventionId}`, { image_signature: imageBase64 });
      setIsSigned(true);
      setSuccess("Signature enregistrée et horodatée avec succès.");
    } catch (err) { setError("Erreur lors de l'enregistrement de la signature."); }
  };
      ...
      {!isSigned ? (
        <div>
          ...
          <div className="mt-3 flex gap-3">
            <button type="button" onClick={clearSig} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 text-sm font-medium border">Effacer</button>
            <button type="button" onClick={saveSig} className="px-6 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 text-sm font-bold">Confirmer la signature</button>
          </div>
        </div>
      ) : (
        <div className="text-center p-6 bg-green-50 border border-green-200 rounded-lg">
          ...
          <p className="text-green-600 text-sm">La signature a été verrouillée et horodatée.</p>
        </div>
      )}
    </div>
  );
